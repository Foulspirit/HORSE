...
#define studentName     "Nikos Kapalos"				//NEW Please change this as appropriate!
...
{ //display garden content on screen
	...
	for (int y(0); y < (SIZEY); ++y)
	{
		...
		{
			gardstr += garden[y][x];			// display current garden contents
		}
		gardstr += "\n";
	}
	printf("%s", gardstr.c_str());
} //end of paintGarden
...
void moveSnail(char foodSources[][SIZEX], int snail[], int keyMove[], string& msg, char garden[][SIZEX], char slimeTrail[][SIZEX])
{
	...
	case LETTUCE:		// increment lettuce count and win if snail is full
		...
		fullOfLettuce ? msg = "LAST LETTUCE EATEN\a\a" : msg = "LETTUCE EATEN\a";
...
{ //show current date and time
	...
	Gotoxy(column, row);
	//cout << "DATE: " << GetDate();
	printer = printer + "DATE: " + GetDate();
	Gotoxy(column, row + 1);
	//cout << "TIME: " << GetTime();
	printer = printer + "TIME: " + GetTime();
...
{ //show game options
	...
	printer = printer + "* TO MOVE USE ARROW KEYS - EAT ALL " + std::to_string( LETTUCE_QUOTA) + " LETTUCES TO WIN.";
	Gotoxy(column, row += 1);
	//cout << "* EAT WORMS (" << WORM << ") AND LETTUCES (" << LETTUCE << ") TO BOOST HEALTH.";
	printer = printer + "* EAT WORMS (" + std::to_string(WORM) + ") AND LETTUCES (" + std::to_string(LETTUCE) + ") TO BOOST HEALTH.";
	Gotoxy(column, row += 1);
	//cout << "* EACH MOVE AND INVISIBLE SLUG PELLETS DEPLETE HEALTH.";
	printer = printer + "* EACH MOVE AND INVISIBLE SLUG PELLETS DEPLETE HEALTH.";
	Gotoxy(column, row += 1);
	//cout << "* TO QUIT ANY TIME USE 'Q'";
	printer += "* TO QUIT ANY TIME USE 'Q'";
